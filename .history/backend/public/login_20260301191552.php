...
if (password_verify($password, $user["password_hash"])) {
    ...
    // Get country using IP API
       ...
       $geo_json = json_decode($geo_data, true);
        ...
        $country = $geo_json["country"] ?? "Unknown";
...
$new_location = ($row_loc["count"] > 0) ? 0 : 1;
    // Insert into DB
    $stmt2 = $conn->prepare("INSERT INTO login_history 
(user_id, ip_address, user_agent, login_time, https_status, new_device, odd_time, country, new_location) 
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
